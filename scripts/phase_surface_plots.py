...
class SweepInterpolator(object):
    ...
    def calculate_growth_on_grid(self, ko=None, N=200):
        ...
        for g, o in itertools.product(glu_grid, ox_grid):
            ...
            phase_df.append((g, o, best_efm, best_gr))
...
def plot_surface(ax, figdata,
    ...
    #X = np.log10(phase_df[D.GLU_COL].as_matrix().reshape(200, 200).T)
    #Y = np.log10(phase_df[D.OX_COL].as_matrix().reshape(200, 200).T)
    phase_df = phase_df.join(figdata, on='best_efm', rsuffix='_')
    ...
    z_mat = phase_df.pivot(index=D.GLU_COL, columns=D.OX_COL, values=z).T.as_matrix()
    ...
    if z == c: # make a standard surface plot with gridlines and big strides
        ...
        c_mat = phase_df.pivot(index=D.GLU_COL, columns=D.OX_COL, values=c).T.as_matrix()
...
def plot_surface_diff(ax, ko_cache_fname, wt_cache_fname='sweep2d_win_200x200.csv'):
    ...
    phase_df, axis_params = get_phase_data(wt_cache_fname)
    wt_gr_mat = phase_df.pivot(index=D.GLU_COL, columns=D.OX_COL,
                               values=D.GROWTH_RATE_L).T.as_matrix()

    phase_df, axis_params = get_phase_data(ko_cache_fname)
    ko_gr_mat = phase_df.pivot(index=D.GLU_COL, columns=D.OX_COL,
                               values=D.GROWTH_RATE_L).T.as_matrix()
...
def plot_heatmap_diff(ax, cache_fname1, cache_fname2, vmax=1):
    phase1_df, wt_axis_params = get_phase_data(cache_fname1)
    gr1_mat = phase1_df.pivot(index=D.GLU_COL, columns=D.OX_COL, values=D.GROWTH_RATE_L).T

    phase2_df, ko_axis_params = get_phase_data(cache_fname2)
    gr2_mat = phase2_df.pivot(index=D.GLU_COL, columns=D.OX_COL, values=D.GROWTH_RATE_L).T
...
def plot_heatmap(ax, wt_cache_fname='sweep2d_win_200x200.csv', vmax=None):
    wt_phase_df, wt_axis_params = get_phase_data(wt_cache_fname)
    wt_gr_mat = wt_phase_df.pivot(index=D.GLU_COL,
...
def get_phase_data(sweep_cache_fname='sweep2d_win_200x200.csv'):
    ...
    # pixel
    phase_df['hexcolor'] = phase_df['best_efm'].apply(D.efm_to_hex)
    ...
    for col in [D.GLU_COL, D.OX_COL]:
        axis_params[col] = {}
        levels = sorted(phase_df[col].unique())
        axis_params[col]['N'] = len(levels)
        axis_params[col]['min'] = phase_df[col].min()
        axis_params[col]['max'] = phase_df[col].max()
        x_std = np.log(standard_conc[col])
        x_min = np.log(phase_df[col].min())
        x_max = np.log(phase_df[col].max())
    ...
    return phase_df, axis_params

def plot_phase_plots(figure_data, sweep_cache_fname='sweep2d_win_200x200.csv'):
    phase_df, axis_params = get_phase_data(sweep_cache_fname)
    ...
    # mark the 3 selected EFMs in the phase plot
    ...
    best_efm_gr_df = phase_df.pivot(index=D.GLU_COL,
                                    columns=D.OX_COL,
                                    values=D.GROWTH_RATE_L)
    ...
    for i, d in enumerate(plot_parameters):
        ...
        ax.annotate(chr(ord('a')+i+2), xy=(0.02, 0.98),
                    ...
                    size=20, color='k')
        ax.set_title(d['c'])
        df = phase_df.join(pareto_data_df[d['c']], on='best_efm')
...
def plot_conc_versus_uptake_figure(figure_data,
    ...
    """
        ...
        join the phase data with the EFM rates table, in order to get specific
        rates for each winning EFM
    """
    phase_df, axis_params = get_phase_data(sweep_cache_fname)
    best_efm_hex = phase_df.pivot(index=D.GLU_COL,
                                  columns=D.OX_COL,
                                  values='hexcolor')
    ...
    Y = np.arange(0, axis_params[D.OX_COL]['N'])
    X, Y = np.meshgrid(X, Y)
    z_mat = phase_df.pivot(index=D.GLU_COL, columns=D.OX_COL, values=D.GROWTH_RATE_L).T.as_matrix()
    ...
    phase_df[D.GROWTH_RATE_L] = phase_df[D.GROWTH_RATE_L].round(3)
    small_phase_df = phase_df[[GLU_UPRATE_L, OX_UPTAKE_L, D.GROWTH_RATE_L, 'hexcolor']].drop_duplicates()
    small_phase_df.sort_values(D.GROWTH_RATE_L, inplace=True)

    ########## 2D scatter plot of uptake rates (winning EFM as color) #########
    ax = fig.add_subplot(2, 2, 3)
    ax_list.append(ax)
    ax.scatter(x=small_phase_df[GLU_UPRATE_L],
               y=small_phase_df[OX_UPTAKE_L],
               s=15, c=small_phase_df['hexcolor'],
               linewidth=0)
    ...
    ax_list.append(ax)
    ax.scatter(xs=small_phase_df[GLU_UPRATE_L],
               ys=small_phase_df[OX_UPTAKE_L],
               zs=small_phase_df[D.GROWTH_RATE_L],
               s=15, c=small_phase_df['hexcolor'],
